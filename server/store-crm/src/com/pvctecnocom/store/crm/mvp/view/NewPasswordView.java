...
import com.pvctecnocom.store.crm.mvp.presenter.password.PasswordNewPresenter;
...
{	
	private PasswordNewPresenter passwordNewPresenter;
    ...
    public NewPasswordView(String code) 
    {    	
    	this.code = code;
    	
        setSizeFull();

        this.passwordNewPresenter = new PasswordNewPresenter();
    ...
    private Component buildFields() 
    {
            ...
            {   
				...
				{	
					...
					{	
						...
						Boolean result = passwordNewPresenter.updatePasswordByCode(code, salt, BCrypt.hashpw(fldPassword.getValue().concat(salt), BCrypt.gensalt(11)));
        ...
        Boolean isValidCode = this.passwordNewPresenter.validarCode(this.code);
        
        if(!isValidCode)
        {
        	btnCambiarPassword.setEnabled(false);
        	
        	String message = "La solicitud de recuperación de contraseña expiró";
